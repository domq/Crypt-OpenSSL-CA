...
upon. Available on L<http://www.openssl.org/> (also accessible through
HTTP/S, of course).
...
=item Parsing a certificate and displaying the details:
...
=item Getting the modulus (unique public key identifier) of a
...
=item Generating a self-signed certificate and matching private key
for tests:
...
=item Building a toy CA:

Under distros that sport a cooperative C</usr/lib/ssl/openssl.cnf> or
C</usr/share/openssl/openssl.cnf>: this was tested on Ubuntu Edgy,
...
roughly the same thing in a more modular although less
feature-complete way.  Seasoned programmers will find the OpenSSL man
pages of some limited help, and the command C<grep -r some_identifier
/usr/include/openssl> to come in handy more often than not.
...
are both databases of L</OID>s that together contain pretty much all
OIDs known to mankind.  The latter sports a search engine.
...
certified the user's certificate, all into a single binary blob.

=item B<RFC2560>

L<Crypt::OpenSSL::CA::Resources/OCSP>
